...
libvlc_media_get_tracks_info( libvlc_media_t *p_md, libvlc_media_track_info_t ** pp_tracks )
{
    ...
    *pp_tracks = (i_tracks > 0) ? malloc( i_tracks * sizeof(libvlc_media_track_info_t) ) : NULL;
    ...
    for( int i = 0; i < i_tracks; i++ )
    {
        ...
        const input_item_track_t *p_track = p_input_item->tracks[i];
        ...
        case VIDEO_ES:
            ...
            p_mes->u.video.i_height = p_track->u.video.i_height;
            p_mes->u.video.i_width = p_track->u.video.i_width;
            break;
        case AUDIO_ES:
            p_mes->i_type = libvlc_track_audio;
            p_mes->u.audio.i_channels = p_track->u.audio.i_channels;
            p_mes->u.audio.i_rate = p_track->u.audio.i_rate;
            break;
        case SPU_ES:
