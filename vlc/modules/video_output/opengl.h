...
#if USE_OPENGL_ES == 2
    supports_npot = true;
#elif defined(__APPLE__) && USE_OPENGL_ES != 1
    supports_npot = true;
#endif
...
#ifdef __APPLE__
        fprintf(stderr, "WARNING: Using a non power of 2 texture\n");
#endif
...
static int vout_display_opengl_ResetTextures(vout_display_opengl_t *vgl)
{
    ...
    for (int i = 0; i < VLCGL_TEXTURE_COUNT; i++) {
        ...
        if (vgl->buffer[i]) {
			glTexImage2D(VLCGL_TARGET, 0, VLCGL_FORMAT, vgl->tex_width, vgl->tex_height, 0, VLCGL_FORMAT, VLCGL_TYPE, vgl->buffer[i]);
		}
    }
                                       ...
                                       picture_t *picture)
{
    ...
    glBindTexture(VLCGL_TARGET, get_texture(picture));
#else
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, vgl->fmt.i_width, vgl->fmt.i_height, VLCGL_FORMAT, VLCGL_TYPE, picture->p[0].p_pixels);
